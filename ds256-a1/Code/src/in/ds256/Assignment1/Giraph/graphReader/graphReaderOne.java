...
public class graphReaderOne extends TextEdgeInputFormat<LongWritable, NullWritable> {
    private static final Pattern SEPARATOR = Pattern.compile("[\t]");

    public graphReaderOne() {
    }
    ...
    public EdgeReader<LongWritable, NullWritable> createEdgeReader(InputSplit split, TaskAttemptContext context) throws IOException {
        return new graphReaderOne.graphReaderOneReader();
    }

    public class graphReaderOneReader extends TextEdgeInputFormat<LongWritable, NullWritable>.TextEdgeReaderFromEachLineProcessed<LongPair> {
        public graphReaderOneReader() {
        }

        protected LongPair preprocessLine(Text line) throws IOException {
            String[] tokens = graphReaderOne.SEPARATOR.split(line.toString());
            try {
                if (tokens.length >= 2)
